...
quotation_list = json.load(file1)
...
for quotation in  quotation_list[1:]:
    ...
    picture = table.cell(1,0)
    picture.text = "圖片"
    product = table.cell(1,1)
    product.text = "產品/規格"
    ...
    for k, v in quotation.items():
        ...
        if j > 3:
            product_content = table.cell(j-2,1)
    ...
    for i in range(len(quotation)-2):
        table.cell(i,0).width = Cm(2.22)
        table.cell(i,1).width = Cm(7.46)
        table.cell(i,2).width = Cm(4.02)
        table.cell(i,3).width = Cm(3.44)
        table.cell(i,4).width = Cm(1.52)
...
2. 匯款資訊：
   戶名：循拾股份有限公司
   國泰世華(013)，敦化分行(212)，帳號：212035007315
"""
elif total >= 10000:
...
contact_content.text = "Joan"

phone = table.cell(1,2)
phone.text = "Email："
phone_content = table.cell(1,3)
phone_content.text = "joan@loopick.com.tw"
